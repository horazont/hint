...
static inline void align_time(struct tm *time)
{
...
static const coord_int_t interval_height_with_space = 64+2;
...
void screen_weather_update(struct screen_t *screen)
{
    ...
    for (int i = 0; i < WEATHER_TIMESLOTS; i++)
    {
        struct weather_interval_t *src = array_get(
            &weather->request_array, i);
        struct weather_info_t *dst = &weather->timeslots[i];

        memcpy(&dst->interval, src, sizeof(struct weather_interval_t));
